...
-- 1. 学校租户主表与入驻申请表
-- 2. 学校级 SSO 配置与学校通知表
-- 3. 为模块01中的学校业务表补充 school_id 外键约束
-- 不负责：
-- 1. 用户角色与认证日志表
...
-- school_notifications：学校生命周期通知记录表，仅保存已有学校租户的到期提醒、冻结通知等发送流水。
...
COMMENT ON TABLE school_notifications IS '学校生命周期通知记录表，仅保存已有学校租户的短信或提醒发送流水。';
COMMENT ON COLUMN school_notifications.type IS '通知类型：1到期提醒 2缓冲期通知 3冻结通知 4审核通过。';
COMMENT ON COLUMN school_notifications.is_sent IS '是否已经发送。';

-- users.school_id 允许 super_admin 使用 0 作为平台级哨兵值，因此不对 users 建立数据库外键。
