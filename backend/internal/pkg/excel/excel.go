...
// SheetConfig 单个工作表导出配置
// 用于在一个 Excel 文件中导出多个业务视图，避免上层重复拼装 workbook 逻辑。
...
func Export(config *ExportConfig, rows [][]interface{}) (*bytes.Buffer, error) {
	return ExportWorkbook([]*SheetConfig{
		{
...
// ExportWorkbook 导出包含多个工作表的 Excel 文件。
// 当业务需要在同一报表中同时交付多个统计视图时，应优先复用这里，而不是在模块中自行操作 excelize。
func ExportWorkbook(sheets []*SheetConfig) (*bytes.Buffer, error) {
	f := excelize.NewFile()
	defer f.Close()
	...
	if len(sheets) == 0 {
		sheets = []*SheetConfig{{SheetName: "Sheet1"}}
	}

	defaultSheet := f.GetSheetName(0)
	for index, sheetConfig := range sheets {
		sheetName := "Sheet1"
		if sheetConfig != nil && sheetConfig.SheetName != "" {
		...
		// 写入表头
		for i, header := range sheetConfig.Headers {
			...
			if i < len(sheetConfig.ColWidths) && sheetConfig.ColWidths[i] > 0 {
				f.SetColWidth(sheetName, colName, colName, sheetConfig.ColWidths[i])
			} else {
		...
		// 写入数据行
		for rowIdx, row := range sheetConfig.Rows {
...
func ImportRawRows(filename string, reader io.Reader) ([][]string, error) {
	...
	case ".xlsx", ".xlsm", ".xltx", ".xltm", "":
		f, err := excelize.OpenReader(reader)
		if err != nil {
			return nil, fmt.Errorf("仅支持.xlsx和.csv格式")
		}
		defer f.Close()
