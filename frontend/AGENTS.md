...
- 前端所有用户可见文案必须使用面向最终用户的产品语义，不得直接暴露模块编号、系统分层、内部对象建模、表名、字段名或实现细节。
- 页面标题、导航标题、按钮、空状态、错误提示、帮助说明和成功反馈必须围绕用户目标表达，而不是围绕研发结构表达。
- 路由、接口、类型、hooks、stores、变量和其他不会直接暴露给用户的内部命名可保持研发语义；本约束仅作用于用户可见表达层。
...
- 路由路径允许保留研发或领域命名，但导航标签、页面主标题和说明文案必须转换为用户可理解的产品表达。
- 禁止把“模块”“租户”“实例生命周期”“内部接口”“配置分组”等内部设计语言直接展示给最终用户。
...
- 加载态、空状态、错误态和成功反馈不得直接引用内部表名、字段名、日志名、协议名或实现细节。
- 平台级专业页面允许保留必要专业词，但必须保持产品化表达，例如优先使用“平台运行”“运行看板”“规则设置”“数据概览”等说法，而不是“治理面板”“模块统计”“内部配置项”等说法。
