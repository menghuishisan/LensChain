...
/**
 * AdminLoginLogPanel 学校管理员登录日志查询组件。
 */
export function AdminLoginLogPanel() {
...
/**
 * SuperLoginLogPanel 超级管理员登录日志查询组件。
 */
export function SuperLoginLogPanel() {
  const [params, setParams] = useState<LoginLogParams>({ page: 1, page_size: 20 });
  const query = useLoginLogs(params);
  const list = query.data?.list ?? [];
...
/**
 * AdminOperationLogPanel 学校管理员操作日志查询组件。
 */
...
/**
 * SuperOperationLogPanel 超级管理员操作日志查询组件。
 */
export function SuperOperationLogPanel() {
  const [params, setParams] = useState<OperationLogParams>({ page: 1, page_size: 20 });
  const query = useOperationLogs(params);
  const list = query.data?.list ?? [];
